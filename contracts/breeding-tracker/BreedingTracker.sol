  ...
  AccessControlUpgradeable
{
  ...
  function initialize(address chickToken) public initializer {
  ...
  function safeRegisterBreeding(uint256 chick1, uint256 chick2)
    external
    override
    onlyRole(BREEDER_ROLE)
  {
    require(
      _chickToken.ownerOf(chick1) == _chickToken.ownerOf(chick2),
      "Chick1 and chick2 must be owned by the same owner"
    );
    require(
      breedings[chick1].length < BREEDING_LIMIT,
      "First chick cannot breed anymore"
    );
    require(
      breedings[chick2].length < BREEDING_LIMIT,
      "Second chick cannot breed anymore"
    );
    require(chick1 != chick2, "Chick1 and chick2 cannot be the same");
    for (uint256 i = 0; i < breedings[chick1].length; i = i.add(1)) {
      require(
        breedings[chick1][i] != chick2,
        "Chick1 and chick2 have already breeded"
      );
    }
    for (uint256 i = 0; i < breedings[chick2].length; i = i.add(1)) {
      require(
        breedings[chick2][i] != chick1,
        "Chick1 and chick2 have already breeded"
      );
    }
  ...
  function _setChickToken(address chickToken) internal {
    ...
    require(
      chickToken.supportsInterface(type(IChickToken).interfaceId),
      "ChickToken does not support IChickToken interface"
    );
